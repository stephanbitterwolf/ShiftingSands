...
Reefs_and_Erosion_w_Geometry<-Reefs_and_Erosion_w_Geometry%>%
  ...
  mutate(Reef_Width = rowSums(pick(Zone_Width_Reef_Flat:Zone_Width_Fore_Reef), na.rm = T),
         Min_Reef_Depth = min(pick(Min_Depth_Reef_Crest:Min_Depth_Fore_Reef), na.rm = T ))%>%
  ungroup()
...
```{r Plotting Dataset}
#Reef vs. Non-Reef Transects
table(st_drop_geometry(data)%>%
     select(Transects_ID, Transect_Type)%>%
     distinct()%>%
     select(Transect_Type))
table(st_drop_geometry(data)%>%
     select(Transects_ID, Transect_Type, Reef_Zones)%>%
     distinct()%>%
     select(Reef_Zones))
...
data<-data%>%
  filter(Reef_Zones%in% c("Flat and Fore Reef",
  "Flat and Crest",
  #"Crest and Fore Reef",
  #"Flat Only",
  #"Crest Only",
...
st_write(Reef_data, here("2_output", "modified_shapefiles", "joined_databases", "Reef_data_aggregated-flat.gpkg"), delete_layer = TRUE)
write_csv(Reef_data,here("2_output", "dataframes","aggregated_data-flat.csv") )
