---
title: "01_Erosion_Model_and_Autocorrelation"
...
Reef_data_aggregated<-st_join(weighted_mean,summarised_dataset, by="reef_ID")%>%
  dplyr::select(-c(reef_ID.y, transformed_uncertainty,Suncert_m))%>%
...
Reef_data_aggregated <- Reef_data_aggregated %>%
  dplyr::select(-c(median_SRate_m, reef_ID))
...
# Extract the first point from each line geometry
first_points <- st_cast(st_line_sample(dataset, sample=0), "POINT")
# Create a new sf object combining the points with the original data
geo_data_map <- st_sf(st_drop_geometry(dataset), geometry = first_points)
